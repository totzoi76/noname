...
if (isset($_GET["catID"])) {
    ...
    // Get Category Info
    $stmt = $conn->prepare("SELECT * FROM `category` WHERE category_id = $catID");
    $stmt->execute([$catID]);
    $category = $stmt->fetch();

    // Get Latest articles
    $stmt = $conn->prepare("SELECT * FROM `article` INNER JOIN category ON id_categorie=category_id WHERE id_categorie = $catID  ORDER BY `article_created_time` DESC ");
    $stmt->execute([$catID]);
